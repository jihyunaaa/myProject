...
@Repository("mySQLProductSingleDAO")
public class MySQLProductSingleDAO { // 개별 상품 기능 - MySQL DB
	private Connection conn;
	private PreparedStatement pstmt;

	// 1. 상품 카테고리(핫이슈, +1 증정상품)
	final String SELECTALLCATEGORY = "SELECT PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_PRICE, "
			+ "PRODUCT_SINGLE_IMAGE, COUNT(PRODUCT_SINGLE_NUMBER) OVER() AS TOTAL_COUNT_NUMBER "
			+ "FROM PRODUCT_SINGLE "
			+ "WHERE PRODUCT_SINGLE_STORE = ? AND PRODUCT_SINGLE_CATEGORY = ? LIMIT ? OFFSET ?";

	// 2. 상품이름 검색(인기순)
	final String SELECTALLSEARCHPOPULAR = "SELECT PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_PRICE, "
			+ "PRODUCT_SINGLE_IMAGE, COUNT(PRODUCT_SINGLE_NUMBER) OVER() AS TOTAL_COUNT_NUMBER"
			+ " FROM PRODUCT_SINGLE WHERE PRODUCT_SINGLE_STORE = ? AND PRODUCT_SINGLE_NAME LIKE CONCAT('%', ?, '%') LIMIT ? OFFSET ?";

	// 3. 상품 전체+인기순
	final String SELECTALLPOPULAR = "SELECT PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME,PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE_IMAGE, "
			+ "COUNT(PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER) OVER() AS TOTAL_COUNT_NUMBER, COUNT(PURCHASE_DETAIL.PURCHASE_DETAIL_NUMBER) AS PURCHASE_COUNT "
			+ "FROM PRODUCT_SINGLE LEFT JOIN PURCHASE_DETAIL ON PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER = PURCHASE_DETAIL.PRODUCT_SINGLE_NUMBER "
			+ "WHERE PRODUCT_SINGLE_STORE = ? "
			+ "GROUP BY PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE_IMAGE "
			+ "ORDER BY PURCHASE_COUNT DESC LIMIT ? OFFSET ?";

	// 4. 상품 전체+가격 높은순
	final String SELECTALLPRICEDESC = "SELECT PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE_IMAGE, "
			+ "COUNT(PRODUCT_SINGLE_NUMBER) OVER() AS TOTAL_COUNT_NUMBER "
			+ "FROM PRODUCT_SINGLE WHERE PRODUCT_SINGLE_STORE = ? "
			+ "ORDER BY PRODUCT_SINGLE_PRICE DESC LIMIT ? OFFSET ?";

	// 5. 상품 전체+가격 낮은순
	final String SELECTALLPRICEASC = "SELECT PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE_IMAGE, "
			+ "COUNT(PRODUCT_SINGLE_NUMBER) OVER() AS TOTAL_COUNT_NUMBER "
			+ "FROM PRODUCT_SINGLE WHERE PRODUCT_SINGLE_STORE = ? "
			+ "ORDER BY PRODUCT_SINGLE_PRICE ASC LIMIT ? OFFSET ?";

	// 6. 상품 카테고리+인기순
	final String SELECTALLCATEGORYPOPULAR = "SELECT PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE_IMAGE, "
			+ "COUNT(PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER) OVER() AS TOTAL_COUNT_NUMBER, COUNT(PURCHASE_DETAIL.PURCHASE_DETAIL_NUMBER) AS PURCHASE_COUNT "
			+ "FROM PRODUCT_SINGLE LEFT JOIN PURCHASE_DETAIL ON PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER = PURCHASE_DETAIL.PRODUCT_SINGLE_NUMBER "
			+ "WHERE PRODUCT_SINGLE_STORE = ? AND PRODUCT_SINGLE_CATEGORY = ? "
			+ "GROUP BY PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE_IMAGE "
			+ "ORDER BY PURCHASE_COUNT DESC LIMIT ? OFFSET ?";

	// 7. 상품 카테고리+가격 높은순
	final String SELECTALLCATEGORYPRICEDESC = "SELECT PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE_IMAGE, "
			+ "COUNT(PRODUCT_SINGLE_NUMBER) OVER() AS TOTAL_COUNT_NUMBER "
			+ "FROM PRODUCT_SINGLE WHERE PRODUCT_SINGLE_STORE = ? AND PRODUCT_SINGLE_CATEGORY = ? "
			+ "ORDER BY PRODUCT_SINGLE_PRICE DESC LIMIT ? OFFSET ?";

	// 8. 상품 카테고리+가격 낮은순
	final String SELECTALLCATEGORYPRICEASC = "SELECT PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE_IMAGE, "
			+ "COUNT(PRODUCT_SINGLE_NUMBER) OVER() AS TOTAL_COUNT_NUMBER "
			+ "FROM PRODUCT_SINGLE WHERE PRODUCT_SINGLE_STORE = ? AND PRODUCT_SINGLE_CATEGORY = ? "
			+ "ORDER BY PRODUCT_SINGLE_PRICE ASC LIMIT ? OFFSET ?";

	// 9. 상품이름 검색(가격 높은 순)
	final String SELECTALLSEARCHPRICEDESC = "SELECT PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE.PRODUCT_SINGLE_NAME, "
			+ "PRODUCT_SINGLE.PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE.PRODUCT_SINGLE_IMAGE, "
			+ "COUNT(PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER) OVER() AS TOTAL_COUNT_NUMBER FROM PRODUCT_SINGLE "
			+ "WHERE PRODUCT_SINGLE_STORE = ? AND PRODUCT_SINGLE.PRODUCT_SINGLE_NAME LIKE CONCAT('%', ?, '%') "
			+ "ORDER BY PRODUCT_SINGLE.PRODUCT_SINGLE_PRICE DESC LIMIT ? OFFSET ?";

	// 10. 상품이름 검색(가격 낮은 순)
	final String SELECTALLSEARCHPRICEASC = "SELECT PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE.PRODUCT_SINGLE_NAME, "
			+ "PRODUCT_SINGLE.PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE.PRODUCT_SINGLE_IMAGE, "
			+ "COUNT(PRODUCT_SINGLE.PRODUCT_SINGLE_NUMBER) OVER() AS TOTAL_COUNT_NUMBER FROM PRODUCT_SINGLE "
			+ "WHERE PRODUCT_SINGLE_STORE = ? AND PRODUCT_SINGLE.PRODUCT_SINGLE_NAME LIKE CONCAT('%', ?, '%') "
			+ "ORDER BY PRODUCT_SINGLE.PRODUCT_SINGLE_PRICE ASC LIMIT ? OFFSET ?";

	// 11. 개별상품 재고순 정렬
	final String SELECTALLSTOCKDESC = "SELECT PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE_STOCK, PRODUCT_SINGLE_IMAGE, "
			+ "PRODUCT_SINGLE_STORE, PRODUCT_SINGLE_CATEGORY, PRODUCT_SINGLE_INFORMATION, "
	...
	// 12. 상품 상세정보
	final String SELECTONE = "SELECT PRODUCT_SINGLE_NUMBER, PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_CATEGORY, PRODUCT_SINGLE_PRICE, PRODUCT_SINGLE_IMAGE, "
			+ "PRODUCT_SINGLE_STOCK, PRODUCT_SINGLE_INFORMATION "
	...
	// 14. 개별상품 재고 및 카테고리 정보
	final String SELECTONESTOCKANDCATEGORY = "SELECT PRODUCT_SINGLE_STOCK, PRODUCT_SINGLE_CATEGORY"
			+ " FROM PRODUCT_SINGLE WHERE PRODUCT_SINGLE_NUMBER = ?";

	// 15. 개별상품 가격 및 이름
	final String SELECTONESINGLEDISCOUNTEDPRICEANDSINGLENAME = "SELECT PRODUCT_SINGLE_NAME, PRODUCT_SINGLE_PRICE"
			+ " FROM PRODUCT_SINGLE WHERE PRODUCT_SINGLE_NUMBER = ?";
	
	// 16. 개별상품 재고 변경
	final String UPDATESTOCKONLY = "UPDATE PRODUCT_SINGLE"
			+ " SET PRODUCT_SINGLE_STOCK = PRODUCT_SINGLE_STOCK-? WHERE PRODUCT_SINGLE_NUMBER = ?";
	...
	public ProductSingleVO getOne(ProductSingleVO productSingleVO) {
		...
		try { // 에러 가능성 있는 코드
			...
			// 개별상품 재고 및 카테고리 정보
			else if (productSingleVO.getCondition().equals("SELECTONESTOCKANDCATEGORY")) {
				pstmt = conn.prepareStatement(SELECTONESTOCKANDCATEGORY); // 구문 저장
				pstmt.setLong(1, productSingleVO.getProductSingleNumber()); // 파서 값 저장
			}
			// 개별상품 가격 및 이름
			else if (productSingleVO.getCondition().equals("SELECTONESINGLEDISCOUNTEDPRICEANDSINGLENAME")) {
				pstmt = conn.prepareStatement(SELECTONESINGLEDISCOUNTEDPRICEANDSINGLENAME); // 구문 저장
				pstmt.setLong(1, productSingleVO.getProductSingleNumber()); // 파서 값 저장
			}
			// 결과 rs에 저장
			ResultSet rs = pstmt.executeQuery();
	...
	private boolean existsByName(String productName) {
		...
		// 한번만 사용하기 때문에 내부에 사용
		try (Connection conn = MySQLJDBCUtil.connect(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
	...
	public boolean update(ProductSingleVO productSingleVO) {
		System.out.println("[update] 개별 상품 수정 시작");
		try {
			conn=MySQLJDBCUtil.connect();
			System.out.println("[update] DB 연결 완료");
			// 개별상품 재고 변경
			if(productSingleVO.getCondition().equals("UPDATESTOCKONLY")) {
				pstmt=conn.prepareStatement(UPDATESTOCKONLY);
				pstmt.setInt(1, productSingleVO.getProductSingleCount());
				pstmt.setLong(2, productSingleVO.getProductSingleNumber());
			}
