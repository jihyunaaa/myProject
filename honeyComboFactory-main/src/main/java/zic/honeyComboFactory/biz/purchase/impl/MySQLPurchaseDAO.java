...
@Repository("mySQLPurchaseDAO")
public class MySQLPurchaseDAO { // 주문 기능 - MySQL DB
	...
	final String SELECTALLONEPERSON = "SELECT PURCHASE.PURCHASE_NUMBER, PURCHASE.PURCHASE_TERMINAL_ID, PURCHASE.PURCHASE_TOTAL_PRICE, "
			+ "PURCHASE.MEMBER_NUMBER, COUNT(*) OVER (PARTITION BY PURCHASE.MEMBER_NUMBER) AS TOTAL_COUNT_NUMBER "
			+ "FROM PURCHASE "
			+ "WHERE PURCHASE.MEMBER_NUMBER = ? ORDER BY PURCHASE.PURCHASE_NUMBER DESC LIMIT ? OFFSET ?";
	...
	// 6. 주문 정보 1개 조회
	final String SELECTONE = "SELECT PURCHASE_NUMBER, PURCHASE_TERMINAL_ID, PURCHASE_TOTAL_PRICE FROM PURCHASE WHERE PURCHASE_NUMBER = ?";
	...
	public boolean insert(PurchaseVO purchaseVO) {
		...
		try {
			...
			pstmt.setLong(1, purchaseVO.getPurchaseNumber());
			pstmt.setString(2, purchaseVO.getPurchaseTerminalId());
			pstmt.setLong(3, purchaseVO.getPurchaseTotalPrice());
			pstmt.setLong(4, purchaseVO.getMemberNumber());
