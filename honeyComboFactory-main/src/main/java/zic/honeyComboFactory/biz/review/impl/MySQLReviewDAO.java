...
@Repository("mySQLReviewDAO")
public class MySQLReviewDAO { // 리뷰 기능 - MySQL DB
	...
	public List<ReviewVO> getAll(ReviewVO reviewVO) {
		...
		try {
			...
			if(reviewVO.getCondition().equals("SELECTALLREVIEWLIST")) {
				...
				System.out.println("[M 로그] SELECTALLREVIEWLIST 실행: index=["+reviewVO.getReviewIndex()+"], count=["+reviewVO.getReviewContentCount()+"]");
			}
			// 한 상품에 대한 리뷰 목록 
				...
				System.out.println("[M 로그] SELECTALLREVIEWONEPRODUCT 실행: comboNumber=["+reviewVO.getProductComboNumber()+"], index=["+reviewVO.getReviewIndex()+"], count=["+reviewVO.getReviewContentCount()+"]");
			}
	...
	public ReviewVO getOne(ReviewVO reviewVO) {
		...
		try {
			...
			if(reviewVO.getCondition().equals("SELECTONEREVIEW")) {
				...
				System.out.println("[M 로그] SELECTONEREVIEW 조건 실행 : memberNumber=["+reviewVO.getMemberNumber()+"], productComboNumber=["+reviewVO.getProductComboNumber()+"]");
			}
			ResultSet rs=pstmt.executeQuery();
			if(rs.next()) {
				data = new ReviewVO();
	...
	public boolean insert(ReviewVO reviewVO) {
		try {
			...
			System.out.println("[M 로그] INSERTREVIEW 실행 : reviewScore ="+reviewVO.getReviewScore()+"], memberName=["+reviewVO.getMemberName()+"], memberNumber=["+reviewVO.getMemberNumber()+"], productComboNumber=["+reviewVO.getProductComboNumber()+"]");
	...
	public boolean update(ReviewVO reviewVO) {
		try {
			...
			System.out.println("[M 로그] UPDATEREVIEW 실행 : reviewScore =["+reviewVO.getReviewScore()+"], reviewContent =["+reviewVO.getReviewContent()+"], reviewNumber =["+reviewVO.getReviewNumber()+"]");
			int rs = pstmt.executeUpdate();
