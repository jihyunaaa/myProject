...
const loadMoreComboUrl = "/product/order"; // 꿀조합 상품 불러오기 서블릿 url
const searchComboProductNameUrl = "/product/search"; // 꿀조합 상품 이름 검색 서블릿 url
// 전역 객체 선언
const productState = {
	...
	response.forEach(productData => {
		...
		content += `
			<div class="col-xl-4 col-lg-4 col-md-6 col-sm-6">
				<div class="single-popular-items mb-50 text-center">
					<div class="popular-img">
						...
						data-combo="true" data-stock="`+ productData.productComboStock + `">
							<span>장바구니 담기</span>
						</div>
					</div>
			...
			+ productData.productComboName + `</a>
						...
						<span>`+ productData.productComboPrice.toLocaleString() + `원</span>
		`;

		// 할인 상품이라면
		if (0 < productData.productComboDiscount) {
			content += `
				<span style="color:red;">-`+ productData.productComboDiscount + `%</span>
				<span style="color:blue;">`+ productData.productComboDiscountedPrice.toLocaleString() + `원</span>
			`
		}
